...
def find_shared_dims_llama(layer, k):
    """Find shared dims using BASE LLAMA contrasts (same as 03_causal_ablation.py)."""
...
def run_condition(model, tok, prompts, label, mx):
    ...
    for p in tqdm(prompts, desc=label):
        try:
            ...
            r = f"[ERROR: {e}]"
        rows.append({**p, "response": r, "condition": label, "model": "med42"})
    return rows

def main():
    args = parse_args()
    shared, ch_ids, mi_ids = find_shared_dims_llama(args.layer, args.k)
    print(f"Layer {args.layer}: {len(shared)} shared dims (Llama contrasts; applied to Med42)")
    ...
    print("Loading m42-health/Llama3-Med42-8B...")
    tok = AutoTokenizer.from_pretrained("m42-health/Llama3-Med42-8B",
    ...
    rows = []
    rows += run_condition(model, tok, all_p, "med42_baseline", args.max_new_tokens)

    print(f"Med42 shared-dim ablation: layer {args.layer}, {len(shared)} dims")
    h = model.model.layers[args.layer].register_forward_hook(make_hook(shared, model.device))
    try:
        rows += run_condition(model, tok, all_p, "med42_shared", args.max_new_tokens)
    finally:
        h.remove()

    print(f"Med42 random ablation: layer {args.layer}, {len(random_dims)} dims")
    h = model.model.layers[args.layer].register_forward_hook(make_hook(random_dims, model.device))
    try:
        rows += run_condition(model, tok, all_p, "med42_random", args.max_new_tokens)
    finally:
        h.remove()
